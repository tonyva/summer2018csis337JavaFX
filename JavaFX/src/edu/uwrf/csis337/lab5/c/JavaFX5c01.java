package edu.uwrf.csis337.lab5.c;

/**
 * JavaFX5c01 - JavaFX program with a canvas image mapped onto a 3D shape
 *   
...
public class JavaFX5c01 extends Application {
	
	private final String TITLE = "JavaFX5c01: image mapped onto a 3D Mesh Shape";
	...
	private Group createCuboid( Point3D p1 ) {
		...
		mymaterial.setSelfIlluminationMap( null /* myimage*/ );
...
//		mymaterial.setSpecularMap( myimage );
		...
		/*
		 * Add an ambient and directional light sources
		 */
		AmbientLight blackAmbientlight = new AmbientLight( Color.BLACK ); blackAmbientlight.setLightOn( true );
		AmbientLight blueAmbientlight  = new AmbientLight( Color.BLUE  ); blueAmbientlight .setLightOn( false );
		AmbientLight redAmbientlight   = new AmbientLight( Color.RED   ); redAmbientlight  .setLightOn( false );
		AmbientLight whiteAmbientlight = new AmbientLight( Color.WHITE ); whiteAmbientlight.setLightOn( true );
		PointLight   blackPointlight   = new PointLight  ( Color.BLACK ); blackPointlight  .setLightOn( true );
		PointLight   bluePointlight    = new PointLight  ( Color.BLUE  ); bluePointlight   .setLightOn( false );
		PointLight   redPointlight     = new PointLight  ( Color.RED   ); redPointlight    .setLightOn( false );
		PointLight   whitePointlight   = new PointLight  ( Color.WHITE ); whitePointlight  .setLightOn( false );
		
		Group result = new Group( meshview,
				blackAmbientlight, blueAmbientlight, redAmbientlight, whiteAmbientlight,
				blackPointlight,   bluePointlight,   redPointlight,   whitePointlight );
		return result;
	}
